...
| `test <skill>` | Run sequential paired baseline/skill tests with the experimental Codex runner |

Common flags: `search --provider skills.sh`, `install --frozen`, `update --dry-run`, `update --latest --save --yes`, `doctor --fix`, `sync --project`, `sync --agent codex`, `sync --skill name`, `sync --replace-unmanaged --yes`, `plugin add --dry-run`, `backup restore --dry-run`, `audit --format sarif --output results.sarif`, `test --runs 3 --model <model>`, `test --trust-tests`, and `import --dry-run`.
