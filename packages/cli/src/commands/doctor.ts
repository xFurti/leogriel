...
import { loadConfig, getRegisteredAdapters, lockToSkillTargets } from '@skillctl/core';
...
export function registerDoctor(program: Command): void {
  program
    ...
    .action(async (options) => {
      ...
      if (options.fix && lock) {
        const res = await syncSkillsToAgents(lockToSkillTargets(lock));
        info.push(`--fix: re-synced ${res.synced} targets`);
      }

      const report = {
        status: issues.length ? 'issues' : audit.status === 'warnings' ? 'warnings' : 'ok',
