...
import { getProjectSkillsStore, requireSkillctlProject, writeArtifact } from '@skillctl/core';
import { CodexRunner, loadTestFile, runSkillTests, type SkillTestFile } from '@skillctl/testing';
...
export function registerTest(program: Command, version: string): void {
  const command = program.command('test [skill]').description('Run experimental paired behavioral tests')
    ...
    .option('--keep-workspace', 'persist workspaces under .skillctl/artifacts/test')
    ...
    .action(async (skill, options) => {
      if (!skill) return;
      try {
        if (options.agent !== 'codex') throw new SkillctlError('Only the codex runner is available in 0.9.0', 'UNSUPPORTED_RUNNER', 2);
  ...
  command.command('init <skill>').description('Create a version-1 behavioral test YAML').option('--json').action(async (skill) => {
    try {
      const cwd = await requireSkillctlProject();
      const target = join(cwd, '.skillctl', 'tests', `${safeName(skill)}.yaml`);
...
async function authorizeCommands(testFile: SkillTestFile, trusted: boolean): Promise<void> {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) throw new SkillctlError(`Command assertions require --trust-tests in non-interactive mode:\n${details}`, 'UNTRUSTED_TEST_COMMAND', 2);
  const approved = await confirm({ message: `Test commands can execute arbitrary user code:\n${details}\nContinue?` });
  if (approved !== true) throw new SkillctlError('Command assertions were not authorized', 'UNTRUSTED_TEST_COMMAND', 2);
}

async function findTestFile(cwd: string, skill: string): Promise<string> {
  for (const extension of ['yaml', 'yml']) {
    const path = join(cwd, '.skillctl', 'tests', `${safeName(skill)}.${extension}`);
    if (await access(path).then(() => true).catch(() => false)) return path;
  }
  throw new SkillctlError(`No test file found for ${skill}; run skillctl test init ${skill}`, 'TEST_NOT_FOUND', 2);
}
...
async function findSkillPath(cwd: string, skill: string): Promise<string> {
  ...
  throw new SkillctlError(`Skill not found: ${skill}`, 'SKILL_NOT_FOUND', 2);
}

async function listTestFiles(cwd: string): Promise<string[]> {
  const root = join(cwd, '.skillctl', 'tests');
...
function parsePositive(value: string, flag: string): number {
  ...
  if (!Number.isInteger(number) || number < 1) throw new SkillctlError(`${flag} must be a positive integer`, 'INVALID_OPTIONS', 2);
  return number;
}

function parseRuns(value: string): number {
  const number = parsePositive(value, '--runs');
  if (number > 20) throw new SkillctlError('--runs must be between 1 and 20', 'INVALID_OPTIONS', 2);
  return number;
}
