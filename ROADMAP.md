...
- `skillctl test --compare <git-ref>` and repeatable regression testing.
...
`1.0.0` is released only after those contracts are verified in external repositories with at least two runners. Pre-releases do not imply API stability for `@skillctl/testing`.
