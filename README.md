...
This reduces configuration leakage but is not an absolute security sandbox. Authentication accepts `CODEX_API_KEY` or `OPENAI_API_KEY`; conflicting values fail before execution, and the selected key is exposed only to the Codex process. Opt-in live runs can instead use `SKILLCTL_CODEX_AUTH_MODE=chatgpt` with an explicit, dedicated `SKILLCTL_CODEX_AUTH_HOME` already authenticated by `codex login`; skillctl checks `codex login status`, never falls back to `~/.codex`, and does not copy, log, modify, redact, or delete that profile. Agent subprocesses receive an explicit safe allowlist derived from the isolated environment—enough to find executables and use isolated HOME and temporary directories—while API keys and every non-allowlisted variable remain excluded. If no model is pinned, paired results are useful within that execution but are not declared stable across dates or environments.
