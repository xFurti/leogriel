# Archived skillctl design and implementation history

**Author:** xFurti & Gabry848
**Date:** 2026-07-10
**Status:** Historical reference. This document is not the current Leogriel architecture contract.
**Archived:** 2026-07-18

---

## Last implementation snapshot before archival

This file preserves the original skillctl proposal, rollout plan, collision analysis, and intermediate Leogriel notes. Refer to the root `leogriel-design.md`, `docs/public-contracts.md`, and `docs/migration-1.0.md` for current behavior.
...
- `@leogriel/project-state` serializes project then store operations and journals manifest/lock updates for rollback and recovery.
- First-party `--json` output uses one versioned envelope and exit codes 0 (success), 1 (warning/partial), and 2 (fatal/validation).
- The workspace contains twelve coordinated publishable packages and is tested on Windows, Linux, and macOS with Node 22.13 and Node 24.
...
The sections below preserve the original **skillctl** name, paths, packages, and collision analysis as historical context. The current implementation baseline above and the migration documentation in README take precedence. npm packages are moving to the separate `@leogriel/*` scope; portable manifest and lock schemas remain unchanged.
