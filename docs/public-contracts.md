...
This document records the contracts currently proposed for `1.0.0`. They are implemented in the `1.0.0-beta.4` candidate tree but remain prerelease contracts until `1.0.0-rc.1` is validated. Any incompatible change before the RC must be documented in `CHANGELOG.md` and the migration guide.
