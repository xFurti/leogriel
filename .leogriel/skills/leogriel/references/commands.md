...
Common flags: `search --provider skills.sh`, `install --frozen`, `update --dry-run`, `update --latest --save --yes`, `doctor --fix`, `sync --project`, `sync --agent codex`, `sync --skill name`, `sync --replace-unmanaged --yes`, `plugin add --dry-run`, `backup restore --dry-run`, `audit --format sarif --output results.sarif`, `test --agent codex|claude`, `test --runs 3 --model <model>`, `test --compare <git-ref>`, `test --trust-tests`, `import --dry-run`, and `import --json`.
